...
FROM videos HAVING distance < 25 ORDER BY distance;')->result_array();
    ...
    public function get_nominatif_pusdikarmed() {
    	$document = $this->db->query("SELECT * FROM `documents` WHERE `type`='nominatif_pusdikarmed'")->row_array();
    	if ($document != NULL) {
    		echo $document['path'];
    	}
    }
    
    public function get_sigap_bencana() {
    	$document = $this->db->query("SELECT * FROM `documents` WHERE `type`='sigap_bencana'")->row_array();
    	if ($document != NULL) {
    		echo $document['path'];
    	}
    }
    
    public function get_deputasi() {
    	$document = $this->db->query("SELECT * FROM `documents` WHERE `type`='deputasi'")->row_array();
    	if ($document != NULL) {
    		echo $document['path'];
    	}
    }
    ...
    public function get_locations() {
    	echo json_encode($this->db->query("SELECT * FROM `locations`")->result_array());
    }
    
    public function get_websites() {
    	echo json_encode($this->db->query("SELECT * FROM `websites`")->result_array());
    }
    ...
    public function send_complaint() {
    	...
    	$sql = "INSERT INTO `reports` (`type`, `judul`, `isi`, `date`, `lat`, `lng`, `address`, `instansi_tujuan`, `kategori`) VALUES ('aduan', '" . $judul . "', '" . $isi . "', '" . $tanggal . "', " . $lat . ", " . $lng . ", '" . $address . "', '" . $instansiTujuan . "', '" . $kategori . "')";
    	$this->db->query($sql);
    	$id = $this->db->insert_id();
    	FCM::send_message('/topics/complaint', 1, 1, 'Ada pengaduan baru yang dikirimkan pengguna', 'Klik untuk cek', array(
    		'id' => $id
    	));
    	echo $sql;
    }
    ...
    public function send_suggestion() {
    	...
    	$instansiTujuan = $this->input->post('instansi_tujuan');
    	$kategori = $this->input->post('kategori');
    	$sql = "INSERT INTO `reports` (`type`, `judul`, `isi`, `instansi_tujuan`, `kategori`) VALUES ('saran', '" . $judul . "', '" . $isi . "', '" . $instansiTujuan . "', '" . $kategori . "')";
    	$this->db->query($sql);
    	$id = $this->db->insert_id();
